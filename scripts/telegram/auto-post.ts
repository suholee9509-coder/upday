/**
 * Telegram Auto-Post Script
 *
 * Fetches latest articles from RSS feed and posts to Telegram
 * Usage: npx tsx scripts/telegram/auto-post.ts
 * Dry run: DRY_RUN=true npx tsx scripts/telegram/auto-post.ts
 */

import 'dotenv/config'
import { config } from './config'
import { postFeedItem, testConnection } from './telegram-client'
import { fetchRssFeed } from '../bluesky/rss-parser'
...
async function main() {
  console.log('====================================')
  console.log('Telegram Auto-Post Script')
  ...
  try {
    const { botName, canPost } = await testConnection()
    if (!canPost) {
      console.error('Bot cannot post to channel. Check admin permissions.')
      process.exit(1)
    }
